...
object BreadthFirst {
  def main(args: Array[String]) {
    ...
    def DFS(start: Vertex, g: Graph): List[Vertex] = {
      ...
      def DFSinner(vertex: Vertex, traversed: List[Vertex]): List[Vertex] = {
        if (traversed.contains(vertex))
          traversed
        else {
          val neighbours:List[Vertex] = g(vertex) filterNot traversed.contains
          neighbours.foldLeft(vertex :: traversed)((b,a) => DFSinner(a,b))
        }
      }
    ...
    writter.write(dfsResult.toString())
    writter.close()
